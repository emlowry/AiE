...
// non-integer constants have to be specified in a single translation unit, not in a header.
...
Player::Player(Ball& a_roBall,
	...
	: Sprite(ac_eSide == LEFT ? LEFT_TEXTURE_NAME : RIGHT_TEXTURE_NAME,
	  ...
	  mc_eSide(ac_eSide)
{
	// Nothing else to do here, since all members have been instantiated in the initializer list.
}
...
bool Player::BallIsIncoming() const
{
	...
	// Get the distance between the center of the player and the center of the ball when in contact
	float fOffsetX = (float)(PADDLE_WIDTH + m_roBall.GetDiameter()) / 2;

	// If the player is on the left and the ball is moving left, return false if the ball has moved
	// past the player and true if it hasn't yet.
	if (mc_eSide == LEFT && fBallVelocityX < 0)
		return (fBallPositionX > (m_oPosition.x - fOffsetX));

	// If the player is on the right and the ball is moving right, return false if the ball has
	// moved past the player and true if it hasn't yet.
...
bool Player::CanHit() const
{
	...
	// Get the horizontal and vertical distances between the center of the player and the center of
	// the ball when the two are touching.
	float fOffsetX = (float)(PADDLE_WIDTH + m_roBall.GetDiameter()) / 2;
...
void Player::Hit() const
{
	...
	// If the ball hits one of the player's horizontal edges directly, then its entire velocity is
	// reversed, not just the horizontal component.
...
void Player::Serve() const
{
	...
	// Serve from the edge of the player facing the field, not from the center of the player.
	if (mc_eSide == LEFT)
		oServePosition.x += (float)(PADDLE_WIDTH + uiBallDiameter) / 2;
	else
		oServePosition.x -= (float)(PADDLE_WIDTH + uiBallDiameter) / 2;

	// Serve towards the other player, angled either up or down by random chance.
	XYPair oServeVelocity = { (mc_eSide == LEFT ? 1.0f : -1.0f), (rand()%2 == 0 ? 1.0f : -1.0f) };
...
// Update the player's position based on the elapsed time and the human or AI controller's input.
...
Player::MotionDirection Player::GetAIDirection() const
{
	...
	// If the center of the ball is above the center of the player, move up unless the player is
	// already as far up as it can go.
	float fMinY = mc_iMinY + ((float)PADDLE_HEIGHT / 2);
	if((fBallPositionY < m_oPosition.y) && (m_oPosition.y > fMinY))
		return UP;

	// If the center of the ball is below the center of the player, move down unless the player is
	// already as far down as it can go.
...
Player::MotionDirection Player::GetHumanDirection() const
{
	...
	// If only the "up" key for this side is pressed, move up unless the player is already as far up
	// as it can go.
	float fMinY = mc_iMinY + ((float)PADDLE_HEIGHT / 2);
	if (bUp && (this->m_oPosition.y > fMinY))
		return UP;

	// If only the "down" key for this side is pressed, move down unless the player is already as
	// far up as it can go.
	float fMaxY = mc_iMaxY - ((float)PADDLE_HEIGHT / 2);
