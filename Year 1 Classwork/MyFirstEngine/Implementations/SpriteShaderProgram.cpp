...
const float SpriteShaderProgram::SPRITE_ST_TEX_COORDINATE_DATA[8] = { 0.0f, 0.0f,
                                                                      ...
                                                                      1.0f, 1.0f,
                                                                      0.0f, 1.0f };/**/
const float SpriteShaderProgram:: // X       Y     S     T
    SPRITE_XYST_VERTEX_DATA[16] = { -0.5f,  0.5f, 0.0f, 0.0f,
                                     0.5f,  0.5f, 1.0f, 0.0f,
                                     0.5f, -0.5f, 1.0f, 1.0f,
                                    -0.5f, -0.5f, 0.0f, 1.0f };
const unsigned int SpriteShaderProgram::SPRITE_ELEMENT_DATA[4] = { 0, 1, 3, 2 };
...
SpriteShaderProgram::SpriteShaderProgram()
    : /*m_iTexTransformID( 0 ),/**/ m_iColorID( 0 ), m_iTextureID( 0 ),/*
      ...
      m_uiVertexArrayID ( 0 ), m_uiTexCoordinateBufferID( 0 ), m_uiElementBufferID( 0 ),
...
void SpriteShaderProgram::SetupData()
{
    ...
    glBufferData( GL_ELEMENT_ARRAY_BUFFER, sizeof( SPRITE_ELEMENT_DATA ),
                  SPRITE_ELEMENT_DATA, GL_STATIC_DRAW );

    // Load Vertex buffer
    GLuint uiVBO;
    glGenBuffers( 1, &uiVBO );
    glBindBuffer( GL_ARRAY_BUFFER, uiVBO );
    glBufferData( GL_ARRAY_BUFFER, sizeof( SPRITE_XYST_VERTEX_DATA ),
                  SPRITE_XYST_VERTEX_DATA, GL_STATIC_DRAW );

    // Specify vertex attributes
    GLint iPositionID = glGetAttribLocation( m_uiID, "i_v2Position" );
    glVertexAttribPointer( iPositionID, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), 0 );
    glEnableVertexAttribArray( iPositionID );
    GLint iCoordinateID = glGetAttribLocation( m_uiID, "i_v2TexCoordinate" );
    glVertexAttribPointer( iCoordinateID, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), (void*)(2*sizeof(float)) );
    glEnableVertexAttribArray( iCoordinateID );
...
/*
    ...
    glUniform1i( Instance().m_iTextureID, a_roTexture.Unit() - GL_TEXTURE0 );/*
    ...
    // Resume using previous shader program and texture
